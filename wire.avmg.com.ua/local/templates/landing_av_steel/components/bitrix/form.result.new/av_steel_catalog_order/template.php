<div class="form-catalogFlex">
	...
	<?if($arResult["isFormNote"] == "Y"):?>
		<div id="form-anwer123" hidden>
			<span id="popUp-text-fromForm">
				<br><span>Спасибо за заявку!<br><?=Loc::getMessage("FORM_ORDER_INCORRECT")?></span><br>
			</span>
		</div>
		<script>
			ShowPopUp();changeUrl();
	...
	<div class="text-uppercase text-center title-form-1"><span><?=Loc::getMessage("FORM_ORDER_CALLBACK")?></span></div>
	...
	$('.av-form-wrap-2  input,.av-form-wrap-2 textarea').each(function() {
		...
		<?endif?>
		<span data-form-input-error-123 ><?=Loc::getMessage("FORM_ORDER_INCORRECT")?></span>
		<?
		/* --------------------------------------------------------------------- */
		/* ------------------------------- поля -------------------------------- */
		/* --------------------------------------------------------------------- */
